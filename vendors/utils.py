...
def calculate_quality_rating_avg(vendor):
    quality_rating_avg = PurchaseOrder.objects.filter(vendor=vendor, status='completed', quality_rating__isnull=False).aggregate(
...
def calculate_average_response_time(vendor):
    avg_response_time_seconds = PurchaseOrder.objects.filter(
        ...
        avg_response_time_seconds=Avg(Cast(Extract('response_time', 'seconds'), output_field=FloatField()))
