    ...
    def __call__(self, request):
        ...
        if user.is_authenticated:
            ...
            if authorization_header.startswith('Bearer '):
                ...
                expiry_time = getattr(access_token, 'expiry_time', None)  # Assuming your token has an expiry time attribute
                if expiry_time:
                    # Check if the access token is about to expire
                    if expiry_time <= timezone.now() + timedelta(minutes=5):
                        # Token is about to expire, refresh it
                        new_access_token = self.get_refresh_token(getattr(user, 'refresh_token', None))
                        if new_access_token:
